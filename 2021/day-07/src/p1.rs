...
pub fn find_optimum(
    ...
    min_x: i64,
    max_x: i64,
    fuel_cost: fn(input: &PuzzleInput, x: i64) -> u64,
) -> i64 {
    let prev_x = bisect(min_x, max_x + 1, |x| {
        // The predicate must flip from false to true at the optimum.
        //
        // This means that we can just check if the cost is increasing,
        // because that will be false before the optimum and true after.
...
fn fuel_cost(input: &PuzzleInput, x: i64) -> u64 {
    input.positions.iter().map(|y| x.abs_diff(*y)).sum::<u64>()
}
