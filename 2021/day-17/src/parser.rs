...
pub fn part1(input: &'static str) -> PuzzleInput {
    ...
    let top_left = Coordinate::new(ints[0].min(ints[1]) as i32, ints[2].min(ints[3]) as i32);
    let bottom_right = Coordinate::new(ints[0].max(ints[1]) as i32, ints[2].max(ints[3]) as i32);
